...
meta-analysis is a key statistical methods to combine, summarize and interpret all available evidence pertaining to a clearly defined research field or research question[@lipsey2001][@doingmeta2024].

However, conducting a Meta-Analysis, especially performing complex Meta-Regression, heterogeneity tests, and publication bias analysis, usually requires profound statistical knowledge and programming skills in languages such as R. This technical barrier hinders many domain experts from quickly extracting and synthesizing conclusions from existing literature.

To lower the technical threshold of Meta-Analysis, I designed and implemented a universal interactive Meta-Analysis tool based on R Shiny App (hereinafter referred to as "App"). The main goal of this App is to provide a no-code data upload and column mapping interface. It supports two main types of data, which is continuous and binary data. One-click generation of professional analysis results include effect size summary, forest plot, and funnel plot. This article will provide a detailed description of the design principles and technical implementation of the App, and demonstrate its powerful functions in practical applications through a simulated dataset related to student success in ENV405.
...
![](Fig1.png){fig-align="center" width="313"}

::: {style="text-align: center;"}
Fig1. Main content of the UI interface.
:::
...
The application is designed for flexibility and does not require strict column header naming conventions. It relies on a column-mapping interface instead. However, the uploaded data table must meet the following structural requirements:
...
The subsequent analysis will be based on the binary data, which is related to **student success in ENV405: Advanced Statistics In Environmental Sciences**.

```{r}
library(knitr) 
ba <- read.csv("binary_data.csv", header = TRUE)
knitr::kable(ba, align = 'c')
```

The dataset comprises 10 independent study cohorts, labeled from **ENV405_2016 to ENV405_2025**, representing data from different academic terms. The data structure includes five key variables: **Study** (the cohort identifier); **Passed_QuizGroup** and **Total_QuizGroup** which record the **number of students who passed** and the **total number of students** in the intervention group (those who accessed the quizzes), respectively; and **Passed_NoQuizGroup** and **Total_NoQuizGroup** which correspond to the pass count and total count for the control group (those who did not access the quizzes). This structure allows for the direct calculation of the Odds Ratio (OR) for each individual study.

The effect measure chosen was the OR. All analyses were performed using the default Random Effects Model with the Restricted Maximum Likelihood (REML) method for heterogeneity variance ($\tau^2$) estimation, with proportion of heterogeneity in the total variation ($I^2$) calculated based on the $Q$ statistic.

# Results and Discussions

## Results

We utilized the binary sample data to simulate a meta-analysis examining the effect of using **optional online practice quizzes** on a student's likelihood of **passing the short-answer section of the ENV405 final exam(Fig2)**. The experimental group consisted of students who accessed the quizzes, and the control group were those who did not.

![](Fig2.png)

::: {style="text-align: center;"}
Fig2. Visual summary of the meta-analysis on the effect of practice quizzes on the ENV405 exam pass rate. **a) Summary Result:** The random-effects model yields a significant pooled Odds Ratio (OR = 1.2858, p = 0.0014), with heterogeneity estimated at I² = 0.0%. **b) Forest Plot:** Shows the individual effect sizes and their 95% confidence intervals (CI) for the ten included studies. The diamond at the bottom indicates a significant positive overall effect, with a 95% CI of \[1.1017, 1.5007\]. **c) Funnel Plot:** Used to assess publication bias. The study effect sizes (log OR) are symmetrically distributed around the pooled estimate, consistent with the I² = 0.0% result.
:::

**Analysis and Interpretation:**

1.  **Effect Size:** The meta-analysis, encompassing $k=10$ studies and 4250 total observations, revealed a **highly statistically significant positive effect** of using the optional practice quizzes. The pooled OR was about $1.29$ , with a $95\%$ CI of $[1.1017, 1.5007]$. This indicates that students who used the quizzes had approximately $29\%$ higher odds of passing the short-answer section of the final exam compared to those who did not. The overall effect test ($Z=3.19$) yielded a highly significant $p$-value ($p=0.0014$).

2.  **Heterogeneity:** The analysis showed **no evidence of statistical heterogeneity**. The Cochran's Q test statistic ($Q=1.72$ on 9 degrees of freedom) had a corresponding $p$-value of $0.9952$. This high $p$-value confirms that the observed variation among the 10 study effects is entirely consistent with random chance. Furthermore, the $I^2$ statistic was $0.0\%$, suggesting that **none of the variability** in the outcomes is due to true differences in the effect size across the studies. The random effects model confirmed a $\tau^2$ of $0$.
